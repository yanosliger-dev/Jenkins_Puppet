...
1. Install Puppet (if it isn’t already installed)
2. Configure the official Jenkins package repository
3. Install Java (OpenJDK 17) and Jenkins
4. Force Jenkins to listen on port 8000  
   (explicitly enforced via a systemd override on Ubuntu to avoid distro quirks)
5. Start and enable the Jenkins service
6. Optionally display the Jenkins initial unlock token in a clean, readable message
